...
describe("should 200", () => {
    ...
    beforeAll(async() => {
        ...
        fs.cp(src_file_path, path.join('/tmp', 'file2.jpg'), { recursive: false }, (err) => {
            if (err) {
                console.log(clp, 'Copy err=', err);
            }
        });
        
        if (!fs.existsSync('/tmp/fold8')) {
            fs.mkdirSync('/tmp/fold8');
        }
    ...
    it("should 200", async () => {
        ...
        formData.append('name', 'file2.jpg');
        


        let response = await fetch(prefix + '/api/zip', {
            method:'POST',
            body: formData,
            headers: headers,
        });
    ...
    afterAll(() => {
        ...
        if (fs.existsSync('/tmp/fold8')) {
            fs.rmSync('/tmp/fold8', { recursive: true, force: true });
        }
