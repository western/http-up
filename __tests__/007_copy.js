...
describe('should 200', () => {
    ...
    beforeAll(async () => {
        ...
        if (!fs.existsSync('/tmp/fold7')) {
            fs.mkdirSync('/tmp/fold7');
        }
    ...
    it('should 200', async () => {
        ...
        formData.append('to_path', 'fold7');

        let response = await fetch(prefix + '/api/copy', {
            method: 'POST',
            body: formData,
            headers: headers,
        });
    ...
    afterAll(() => {
        ...
        if (fs.existsSync('/tmp/fold7')) {
            fs.rmSync('/tmp/fold7', { recursive: true, force: true });
        }
    });
});
