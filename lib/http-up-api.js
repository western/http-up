        ...
        if (typeof req.body.fileBase == 'object') {
            ...
            for (let a = 0; a < req.files.fileBlob.length; a++) {
                ...
                if (argv.crypt && code.length > 0) {
                    ...
                    fs.rename(f.path, readFolder + name + '.crypt', (err) => {
                        if (err) {
                            console.log('fs.rename', 'err=', err);
                        } else {
                            let clp = util.common_log_prefix(req);

                            console.log(clp, 'File ' + chalk.green(readFolder + name) + ' was encrypted and saved');
        ...
        if (req.body.name && readFolder.length > 0) {
            ...
            req.body.name.forEach((el, indx) => {
                ...
                if (clean_el && clean_el.length > 0) {
                    ...
                    fs.rename(path.join(readFolder, clean_el), path.join(argv.fold, to, clean_el), () => {});
                }
            });
            ...
            req.body.name.forEach((el, indx) => {
                ...
                if (clean_el && clean_el.length > 0) {
                    ...
                    */
                    ...
                    fs.cp(path.join(readFolder, clean_el), path.join(argv.fold, to, clean_el), { recursive: true }, () => {});
                }
            });
