...
exports.makeServer = function ( argv ) {
    ...
    const upload = multer({ dest: '/tmp', limits: { fieldSize: 100 * 1024 * 1024 } })

    const flUpload = upload.fields([{ name: 'fileBase', maxCount: 10 }, { name: 'fileMeta', maxCount: 10 }])
            ...
            for(let a=0; a<req.body.fileBase.length; a++){
                ...
                fs.writeFile(readFolder + m.name, buf,  "binary",function(err) {
                    if(err) {
                        ...
                        console.log(`The file ${readFolder + m.name} was saved`);
