...
exports.api_core = (app, argv) => {
    app.get('*', async (req, res) => {
        ...
        fs.readdir(readFolder, (err, files) => {
            ...
            let rows = generate_file_rows(req, res, readFolder, req_path, files, sort);
...
let generate_file_rows = (req, res, readFolder, req_path, files, sort) => {
    let file_list1 = [];
    let file_list2 = [];
        ...
        files.forEach((file) => {
            ...
            if (stats.isDirectory()) {
                file_list1.push({
                    ...
                    FullPathPreview: path.join("/__thumb/", req_path, file),
                    Name: file,
        ...
        if (files2) {
            try {
                files2.forEach((file2) => {
                    ...
                    if (stats.isDirectory()) {
                        nodes_ = tree_walk(argv, path.join(fold_name, file2), cnt_deep + 1);

                        let nd = {
                            text: file2,
                            _href: path.join(fold_name, file2),
                            path: path.join(fold_name, file2),
                            icon: 'bi bi-folder',
                            expanded: true,
                            nodes: nodes_,
                        };
