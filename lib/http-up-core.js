...
//let core_tmpl = fs.readFileSync(path.resolve(__dirname, 'core.html'), 'utf8')
//console.log('core_html=', core_tmpl);
...
function error_page_content(h1_title, msg){
...
exports.api_core = function (app, argv) {
    ...
    app.get('*', function (req, res) {
        ...
        readFolder = readFolder.replace(/\.\./g, '');
        ...
        fs.readdir(readFolder, (err, files) => {
            if (err && err.code == 'ENOTDIR') {
                ...
                try {
                    ...
                    if( err.code == 'EACCES' ){
                        res.status(403).send( error_page_content('403', '403 Forbidden') );
                        console.log(clp, '403 Forbidden', colors.yellow(readFolder));
                    }else{
                        res.status(500).send( error_page_content('500', '500 Internal Server Error')  );
            ...
            if (err && err.code == 'ENOENT') {
                ...
                res.status(404).send( error_page_content('404', '404 Not found') );
