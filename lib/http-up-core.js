...
exports.api_core = (app, argv) => {
    app.get('*', async (req, res) => {
        ...
        //let readFolder = path.join(argv.fold, urlencode.decode(req.path));
        //readFolder = util.http_path_clear(readFolder);

        //let rpath = util.http_path_clear(urlencode.decode(req.path));
        //let readFolder = path.join(argv.fold, rpath);
        ...
        files.forEach((filename) => {
            ...
            if (stats.isDirectory()) {
                file_list1.push({
                    ...
                    FullPath: path.join(req_path, filename),
