...
let prepare_file_list_template = (argv, req, res, readFolder, req_path, files) => {
    ...
    let tree_walk = (fold_name) => {

        //console.log('tree_walk call=', fold_name);

        let readFolder2 = path.join(argv.fold, fold_name);
        readFolder2 = util.http_path_clear(readFolder2);
        //console.log('readFolder2=', readFolder2);
            ...
            if(files2){
                try{
                    files2.forEach((file2) => {
                        ...
                        //console.log('try1=', path.join(readFolder2 , file2));
                        let stats = fs.lstatSync(path.join(readFolder2 , file2));
                        let nodes_;

                        if (stats.isDirectory()) {

                            //console.log('try2.1=', fold_name + '/' + file2);
                            //console.log('try2.2=', path.join(fold_name , file2));
                            nodes_ = tree_walk(path.join(fold_name , file2) );
    ...
    try {
        files.forEach((file) => {
            ...
            if (stats.isDirectory()) {
                ...
                let nodes2 = tree_walk(path.join(req_path, file));
