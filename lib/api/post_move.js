...
export const post_move = (app, argv) => {
            ...
            for (let a = 0; a < req.body.name.length; a++) {
                ...
                if (clean_el && clean_el.length > 0) {
                    ...
                    if (!fs.existsSync(src_file_path)) {
                        model.event_log.write(res, req, 404, 'api/move', 'Not found ' + chalk.yellow(src_file_path));
                    ...
                    if (fs.existsSync(target_file_path)) {
                        try {
                            ...
                            model.event_log.write(res, req, 200, 'api/move', 'File/folder ' + target_file_path + ' is exist. It will be replace.');
                        } catch (e) {
                            model.event_log.write(res, req, 500, 'api/move', 'Delete error: ', e.toString());
                    ...
                    try {
                        ...
                        model.event_log.write(res, req, 200, 'api/move', 'Move ' + chalk.yellow(src_file_path) + ' to ' + chalk.yellow(target_file_path));
                    } catch (e) {
                        model.event_log.write(res, req, 500, 'api/move', 'Rename error: ', e.toString());
