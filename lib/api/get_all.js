...
export const get_all = (app, argv) => {
    const err_code_return = (req, res, readFolder, err, is_arr_code) => {
        if (!err) {
            return false;
        }
    ...
    const first_url = (req, res, next) => {
        //console.log('first_url run');
        ...
        if (req_path && req_path.startsWith('/__assets/')) {
            //console.log('ASSETS readFolder=', readFolder);
            //console.log('util.__filename=', util.__filename);
            ...
            if (cntx_arr) {
                ...
                //console.log('ASSETS change to readFolder=', readFolder);
            }
        }
        ...
        res.locals.readFolder = readFolder;
        res.locals.is_no_show_log = is_no_show_log;

        next();
    };

    const second_stat = (req, res, next) => {
        //console.log('second_stat run');

        let readFolder = res.locals.readFolder;

        fs.stat(readFolder, (err, stats) => {
            if (err) {
                const is_problem = err_code_return(req, res, readFolder, err, ['ENOENT', 'EACCES']);
                if (is_problem) {
                    return;
                }
            }
    ...
    const third_file = (req, res, next) => {
        //console.log('third_file run');
        ...
        if (res.locals.stats.isFile()) {
            fs.readFile(readFolder, (err, data) => {
                ...
                let mime_type = mime.getType(readFolder);
    ...
    const fourth_directory = (req, res, next) => {
        //console.log('fourth_directory run');
        ...
        if (fs.existsSync(path.join(readFolder, 'index.html'))) {
            let read_file = path.join(readFolder, 'index.html');
            model.event_log.write(res, req, 200, 'core', 'Index.html found ' + chalk.yellow(read_file));

            fs.readFile(read_file, (err, data) => {
                if (err) {
                    const is_problem = err_code_return(req, res, read_file, err, ['ENOENT', 'EACCES']);
                    if (is_problem) {
                        return;
                    }

                    model.event_log.write(res, req, 500, 'core', 'Internal Server ' + chalk.yellow(read_file) + ' fs.readFile_3388 ' + err);
                    res.status(500).send(util.error_page_content('500', 'Internal Server'));
                    return;
                }

                let mime_type = mime.getType(read_file);
                if (mime_type.includes('text')) {
                    mime_type += '; charset=utf-8';
                }

                if (!is_no_show_log) {
                    model.event_log.write(res, req, 200, 'core', 'SendData ' + chalk.yellow(read_file));
                }
        ...
        fs.readdir(readFolder, (err, files) => {
            if (err) {
                const is_problem = err_code_return(req, res, readFolder, err, ['ENOENT', 'EACCES', 'ENOTDIR']);
                if (is_problem) {
                    return;
                }
...
const prepare_main_template = async (argv, req, res, readFolder, req_path, files) => {
    ...
    if (argv['extend-mode']) {
        ...
        for (let a = 0; a < rows.length; a++) {
            full_filename_arr.push(path.join(argv.fold, rows[a].FullPath));
        }
    ...
    try {
        ...
        files.forEach((filename) => {
            ...
            if (stats.isDirectory()) {
                ...
                full_filename_arr.push(path.join(readFolder, filename));
            }
        });
