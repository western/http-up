        ...
        fs.readdir(readFolder, (err, files) => {
            if (err && err.code == 'ENOTDIR') {
                ...
                if (is_ext_crypt && code && code.length > 0) {
                    ...
                    res.sendFile(readFolder, {  }, function (err) {
                        if (err) {
                            
                            model.event_log().write( req, 500, 'core', '500 Internal Server Error ' + chalk.yellow(readFolder) + ' ' + err );
                            
                            //return res.status(500).send(util.error_page_content('500', '500 Internal Server Error'));
                            //console.log('res.sendFile', readFolder ,' err=', err);
            ...
            if (err && err.code == 'EACCES') {
                ...
                model.event_log().write( req, 403, 'core', '403 Forbidden ' + chalk.yellow(readFolder) );
                return;
            }
