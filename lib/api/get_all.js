...
export const get_all = (app, argv) => {
    ...
    const third_file = async (req, res, next) => {
        ...
        if (res.locals.stats.isFile()) {
            let file_obj = new TFile(readFolder);

            file_obj
                .returnFile(res)
                .then(() => {
                    if (!is_no_show_log) {
                        model.event_log.write(res, req, 200, 'core/3', 'sendFile ' + chalk.yellow(readFolder));
                    }
                })
                .catch((err) => {
    ...
    const fourth_directory = async (req, res, next) => {
        ...
        if (!res.locals.stats.isDirectory()) {
            model.event_log.write(res, req, 500, 'core/4', 'Internal Server Error ' + chalk.yellow(readFolder) + ' IS NOT DIRECTORY');

            res.status(500).send(util.error_page_content('500', 'Internal Server'));
            return;
        }

        let index_file_path = path.join(readFolder, 'index.html');
        ...
        fs.readdir(readFolder)
            .then((files) => {
                if (!is_no_show_log) {
                    model.event_log.write(res, req, 200, 'core/4', 'Dir ' + chalk.yellow(readFolder));
                }
...
const prepare_main_template = async (argv, req, res, readFolder, req_path, files) => {
    ...
    let rows = generate_file_rows(readFolder, req_path, files, prm_sort);
...
const generate_file_rows = (readFolder, req_path, files, prm_sort) => {
    let file_list1 = [];
    let file_list2 = [];

    //try {
    files.forEach((filename) => {
        let fl = new TFile(path.join(readFolder, filename));
