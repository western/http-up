...
export const get_all = (app, argv) => {
    const handleError = (err, res, req, tag) => {
        if (err && err.code && err.code == 'ENOENT') {
            ...
            model.event_log.write(res, req, 404, tag, 'Not found ' + chalk.yellow(readTarget) + ' Request ' + chalk.yellow(req_path));
        ...
        if (err && err.code && err.code == 'EACCES') {
            ...
            model.event_log.write(res, req, 403, tag, 'Forbidden ' + chalk.yellow(readTarget) + ' Request ' + chalk.yellow(req_path));
    ...
    const first_url = async (req, res, next) => {
        ...
        let req_path = util.http_path_clear(urlencode.decode(req.path));
