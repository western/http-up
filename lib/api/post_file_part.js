...
export const post_file_part = (app, argv) => {
    class partUploadEngine {
        constructor(args = {}) {
            ...
            if (!args.parts_fold) {
                throw new Error('partUploadEngine.parts_fold is mandatory arg');
            }

            this.root_fold = args.root_fold;

            this.parts_fold = args.parts_fold;
            this.mkPartsFolder();
        }

        async mkPartsFolder() {
            try {
                await fs.mkdir(this.parts_fold, { recursive: true });
            } catch (err) {
                throw new Error(`Failed to create directory: ${err.message}`);
            }
        }

        async clearAssembledFileParts(req, res, file_id, chunk_cnt) {
            const arr = file_id.split('--');

            for (let i = 0; i < chunk_cnt; i++) {
                let target_file_path = path.join(this.parts_fold, file_id + '.' + i);

                await fs.unlink(target_file_path);
            }

            await fs.unlink(path.join(this.parts_fold, arr[0]));

            model.event_log.write(res, req, 200, 'api/file/part', 'Parts ' + chalk.green(arr[0]) + ' cleared');
        }

        async assembleFile(req, res, file_id, chunk_cnt) {
            const arr = file_id.split('--');

            const writeStream = fsSync.createWriteStream(path.join(this.parts_fold, arr[0]));
            ...
            this.clearAssembledFileParts(req, res, file_id, chunk_cnt);
        }

        async copyAssembledTo(req, res, file_id, to_path) {
            const arr = file_id.split('--');

            let src_file_path = path.join(this.parts_fold, arr[0]);
        ...
        async handleUpload(req, res) {
            ...
            let target_file_path = path.join(this.parts_fold, file_id + '.' + chunk_index);
    ...
    const puEngine = new partUploadEngine({
        root_fold: argv.fold,
        parts_fold: config.httpup_parts,
    });
