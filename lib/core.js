...
exports.makeServer = (argv) => {
    ...
    if (argv.tls) {
        ...
        easyrsa.openssl_make_keys(argv);
        
        
        
        

        console.log('crt:', path.join(httpup_home, '/tls/server.crt'));
        console.log('key:', path.join(httpup_home, '/tls/server.key'));
        console.log('');

        const serverOptions = {
            cert: fs.readFileSync(path.join(httpup_home, '/tls/server.crt')),
            key: fs.readFileSync(path.join(httpup_home, '/tls/server.key')),
